...
class Flux_Template {
    ...
    /**
     * Returns an array of login items that should be displayed from the theme.
     * Only login items the current user (and their group level) have access to
     * will be returned as part of the array;
     *
     * @return array
     */
    public function getLoginItems()
    {
        $auth          = Flux_Authorization::getInstance();
        $defaultAction = Flux_Dispatcher::getInstance()->defaultAction;
        $loginItems    = Flux::config('LoginItems');
        $allowedItems  = array();

        if (!($loginItems instanceOf Flux_Config)) {
            return array();
        }

        foreach ($loginItems->toArray() as $loginLabel => $loginItem) {
            $module = array_key_exists('module', $loginItem) ? $loginItem['module'] : false;
            $action = array_key_exists('action', $loginItem) ? $loginItem['action'] : $defaultAction;
            $param  = array_key_exists('param', $loginItem) ? $loginItem['param'] : array();
